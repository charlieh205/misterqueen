...
void perft_test (char * fen, unsigned long long * expected, int count)
{
    ...
    for (int depth = 0; depth < count; depth++)
    {
        ...
        printf("%s: depth = %2d, time = %.3f, expected = %12llu, actual = %12llu, hits = %g%%\n", result, depth,
               elapsed, expected[depth], actual, 100.0 * hits / actual);
    }
    printf("\n");
}
...
void perft_tests ()
{
    ...
    unsigned long long nodes6[] = {
        1L, 46L, 2079L, 89890L, 3894594L, 164075551L, 6923051137L, 287188994746L, 11923589843526L, 490154852788714L,
    };
