...
#define EMIT_PROMOTIONS(m, a, b)                                                                                       \
    EMIT_PROMOTION(m, a, b, BOARD_QUEEN)                                                                               \
    EMIT_PROMOTION(m, a, b, BOARD_ROOK)                                                                                \
    EMIT_PROMOTION(m, a, b, BOARD_BISHOP)                                                                              \
    EMIT_PROMOTION(m, a, b, BOARD_KNIGHT)
...
{
    return gen_knight_moves(moves, board->white_knights, ~board->white);
}

int gen_white_bishop_moves (Board * board, Move * moves)
{
    return gen_bishop_moves(moves, board->white_bishops, ~board->white, board->all);
}

int gen_white_rook_moves (Board * board, Move * moves)
{
    return gen_rook_moves(moves, board->white_rooks, ~board->white, board->all);
}

int gen_white_queen_moves (Board * board, Move * moves)
{
    return gen_queen_moves(moves, board->white_queens, ~board->white, board->all);
}

int gen_white_king_moves (Board * board, Move * moves)
{
    return gen_king_moves(moves, board->white_kings, ~board->white);
}

int gen_white_king_castles (Board * board, Move * moves)
{
    Move * ptr = moves;
    if (board->castle & BOARD_CASTLE_MASK_WHITE_KING)
    {
    ...
    if (board->castle & BOARD_CASTLE_MASK_WHITE_QUEEN)
    {
...
{
    return gen_knight_moves(moves, board->white_knights, mask);
}

int gen_white_bishop_attacks_against (Board * board, Move * moves, bb mask)
{
    return gen_bishop_moves(moves, board->white_bishops, mask, board->all);
}

int gen_white_rook_attacks_against (Board * board, Move * moves, bb mask)
{
    return gen_rook_moves(moves, board->white_rooks, mask, board->all);
}

int gen_white_queen_attacks_against (Board * board, Move * moves, bb mask)
{
    return gen_queen_moves(moves, board->white_queens, mask, board->all);
}

int gen_white_king_attacks_against (Board * board, Move * moves, bb mask)
{
    return gen_king_moves(moves, board->white_kings, mask);
}
...
{
    return gen_knight_moves(moves, board->black_knights, ~board->black);
}

int gen_black_bishop_moves (Board * board, Move * moves)
{
    return gen_bishop_moves(moves, board->black_bishops, ~board->black, board->all);
}

int gen_black_rook_moves (Board * board, Move * moves)
{
    return gen_rook_moves(moves, board->black_rooks, ~board->black, board->all);
}

int gen_black_queen_moves (Board * board, Move * moves)
{
    return gen_queen_moves(moves, board->black_queens, ~board->black, board->all);
}

int gen_black_king_moves (Board * board, Move * moves)
{
    return gen_king_moves(moves, board->black_kings, ~board->black);
}

int gen_black_king_castles (Board * board, Move * moves)
{
    Move * ptr = moves;
    if (board->castle & BOARD_CASTLE_MASK_BLACK_KING)
    {
    ...
    if (board->castle & BOARD_CASTLE_MASK_BLACK_QUEEN)
    {
...
{
    return gen_knight_moves(moves, board->black_knights, mask);
}

int gen_black_bishop_attacks_against (Board * board, Move * moves, bb mask)
{
    return gen_bishop_moves(moves, board->black_bishops, mask, board->all);
}

int gen_black_rook_attacks_against (Board * board, Move * moves, bb mask)
{
    return gen_rook_moves(moves, board->black_rooks, mask, board->all);
}

int gen_black_queen_attacks_against (Board * board, Move * moves, bb mask)
{
    return gen_queen_moves(moves, board->black_queens, mask, board->all);
}

int gen_black_king_attacks_against (Board * board, Move * moves, bb mask)
{
    return gen_king_moves(moves, board->black_kings, mask);
}
